...
public class BoardFreeRestController implements IResponseController {
    ...
    public ResponseEntity<ResponseDto> addLikeQty(Model model, @Validated @PathVariable Long id) {
        try {
            ...
            this.boardFreeService.addLikeQty(cudInfoDto.getLoginUser(), id);
            IBoardFree result = this.getBoardAndLike(id, cudInfoDto.getLoginUser());
    ...
    public ResponseEntity<ResponseDto> subLikeQty(Model model, @Validated @PathVariable Long id) {
        try {
            ...
            CUDInfoDto cudInfoDto = makeResponseCheckLogin(model);
            this.boardFreeService.subLikeQty(cudInfoDto.getLoginUser(), id);
            IBoardFree result = this.getBoardAndLike(id, cudInfoDto.getLoginUser());
            return makeResponseEntity(HttpStatus.OK, ResponseCode.R000000, "성공", result);
        } catch (LoginAccessException ex) {
            log.error(ex.toString());
